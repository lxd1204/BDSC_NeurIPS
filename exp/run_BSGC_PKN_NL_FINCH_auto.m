...
clear;
clc;
data_path = fullfile(pwd, '..',  filesep, "data_sv", filesep,"test_data_1",filesep);
addpath(data_path);
...
for i1 = 1 : length(datasetCandi)%
    ...
    if ~exist(fname2, 'file')
        ...
        nParam = length(k_range) * length(entropy_range);
            ...
            for iEntropy = 1:length(entropy_range)
                ...
                if exist(fname3, 'file')
                    ...
                    for iRepeat = 1:nRepeat
                        ...
                        e_type = 15;
                        [label, objHistory, result_iter,objHistory_whole,res_whole_aio] = BSGC_entropy_auto_once(label00,Ls, Y0, e_type, Y);
...
%                         plot_converge_1v4(objHistory, result_iter(:, 1), result_iter(:, 2), result_iter(:, 3), result_iter(:, 5), exp_n, data_name, iParam, iRepeat);
                    ...
                    t2 = toc;
                    BSGC_PKN_NL_FINCH_auto_time(iParam) = t0 + t1 + t2/nRepeat;
                    save(fname3, 'result_11_s', 't0', 't2', 't1', 'knn_size', 'e_type','result_all_iter','obj_all_iter','obj_whole_iter','res_whole_iter');
                end
            end
        end
        ...
        save(fname2, 'BSGC_PKN_NL_FINCH_auto_result', 'BSGC_PKN_NL_FINCH_auto_grid_result', 'BSGC_PKN_NL_FINCH_auto_time', 'BSGC_PKN_NL_FINCH_auto_result_summary','label','result_all_iter','obj_all_iter','res_whole_iter','obj_whole_iter');
